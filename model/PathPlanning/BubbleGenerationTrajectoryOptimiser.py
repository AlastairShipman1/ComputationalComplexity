...
def FindClosestWaypointTime(timestamp, waypoints=None):
    if waypoints==None:
        return None
...
def CreateBubbles(waypoints, obstacles, waypoints_other=None):
    bubbles=[]

    bubbles.append(GenerateBubble(waypoints[0], obstacles))
    for i in range(1, len(waypoints)):
        if waypoints_other==None:
            d_wp=np.inf
        else:
...
def main():
    ...
    # PlotEverything([bubbles_1], outline_poly, [route_2], [optimised_1])
